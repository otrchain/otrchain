...
#if defined(NDEBUG)
#error "OTRchain cannot be compiled without assertions."
#endif
...
static unsigned int nStakeMinAgeV1 = 1 * 60 * 60; // 1 hours
static unsigned int nStakeMinAgeV2 = nStakeMinAgeV1; //1 hr
const int targetReadjustment_forkBlockHeight = 300; //retargeting since 700 block
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOTRchain spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zOTRchain spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zOTRchain spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    //POS Starts here
bool phase2 = nHeight > Params().LAST_POW_BLOCK() && nHeight <= 45000;
bool phase3 = nHeight >= 45001 && nHeight <= 90000;



	int64_t nSubsidy = 1 * COIN;
		if (nHeight == 1) return 2000000 * COIN;
	if(phase1)
        nSubsidy = 5.1* COIN;
  else if (phase2)
        nSubsidy = 3* COIN;
...
int64_t GetTreasuryAward(int nHeight)
{
	if(IsTreasuryBlock(nHeight)) {
		if(nHeight == nStartTreasuryBlock)
			return 2 * COIN; //2 for the first treasury block, 10 - reward to PoS
		else{
            // int p = 3/100;
            // int devreward = GetBlockValue(nHeight) / 100 * 3;
			return (GetBlockValue(nHeight) / 100 * 0) * COIN ; //0% of block reward for dev
            }
	} else
		return 0;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OTRchain
...
{
    RenameThread("OTRchain-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOTRMinted()
{
...
void RecalculateZOTRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOTR supply
...
bool RecalculateOTRSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zOTRchain with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zOTRchainSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OTRchain to zOTR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOTRchain spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOTRchain serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOTRchain mints and %d zOTRchain spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OTRchain: We use certain sporks during IBD, so check to see if they are
