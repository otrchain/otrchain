...
#include "zOTRchaincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOTR ought to be enough for anybody." - Bill Gates, 2017
    ui->zOTRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOTRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOTR->setEnabled(false);
        ui->pushButtonMintzOTR->setToolTip(tr("zOTR is currently disabled due to maintenance."));

        ui->pushButtonSpendzOTR->setEnabled(false);
        ui->pushButtonSpendzOTR->setToolTip(tr("zOTR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOTRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOTR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOTR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOTR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOTR in ") +
...
void PrivacyDialog::on_pushButtonSpendzOTR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOTR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOTR
        sendzOTR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOTR
    sendzOTR();
}

void PrivacyDialog::on_pushButtonZOTRchainControl_clicked()
{
    ZOTRchainControlDialog* zOTRchainControl = new ZOTRchainControlDialog(this);
    zOTRchainControl->setModel(walletModel);
    zOTRchainControl->exec();
}

void PrivacyDialog::setZOTRchainControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOTRchainSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOTR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid OTRchain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOTRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOTR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OTRchain </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOTR</b>";
    ...
    // use mints from zOTRchain selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOTRchainControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOTRchainControlDialog::GetSelectedMints();
    }
    ...
    // Clear zOTRchain selector in case it was used
    ZOTRchainControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOTRchain Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " OTRchain, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOTRchain Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOTR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOTR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOTR "));
    ui->labelzOTRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOTRSyncStatus->setVisible(fShow);
}
