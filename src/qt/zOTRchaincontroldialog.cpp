...
#include "ui_zOTRchaincontroldialog.h"
...
std::list<std::string> ZOTRchainControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOTRchainControlDialog::listMints;

ZOTRchainControlDialog::ZOTRchainControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOTRchainControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOTRchainControlDialog::~ZOTRchainControlDialog()
{
    delete ui;
}

void ZOTRchainControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOTRchainControlDialog::updateList()
{
...
void ZOTRchainControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOTRchainControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOTRchain_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOTRchainControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOTRchainControlDialog::GetSelectedMints()
{
...
void ZOTRchainControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
