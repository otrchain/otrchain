...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 OTRchain at the same time
    ONLY_3000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zOTR send
enum ZerocoinSpendStatus {
    ZOTR_SPEND_OKAY = 0,                            // No error
    ZOTR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOTR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOTR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOTR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOTR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOTR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOTR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOTR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOTR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOTR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOTR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOTR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOTR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOTR_SPENT_USED_ZOTR = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZOTRchainAutoBackups(bool fEnabled)
    {
