...
// Copyright (c) 2017-2018 The OTRchain developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total OTRchain balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zOTRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zOTR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zOTR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zOTR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zOTR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zOTR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zOTR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zOTR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zOTR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zOTR denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in OTRchain/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in OTRchain/kb\n"
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zOTRchainObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zOTRchainObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zOTRsupply", zOTRchainObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"OTRchainaddress\"\n"
            "\nReturn information about the given OTRchain address.\n"
            "\nArguments:\n"
            "1. \"OTRchainaddress\"     (string, required) The OTRchain address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"OTRchainaddress\", (string) The OTRchain address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: OTRchain address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are OTRchain addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) OTRchain address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"OTRchainaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"OTRchainaddress\"  (string, required) The OTRchain address to use for the signature.\n"
